...
void AvlTree::calculate_directory_requirements(Node* node, void* context, int depth) {
    ...
    if (node->subdirectory != EMPTY_SUBDIRECTORY) {
        traverse(node->subdirectory, TraversalMethod::PREFIX, [this](Node* node, void* context, int depth) {
            calculate_directory_size(node, static_cast<uint64_t*>(context), depth);
        }, &node->file_size, 0);
...
void AvlTree::calculate_directory_offsets(Node* node, uint64_t* current_sector, int depth) {
    ...
    if (node->subdirectory == EMPTY_SUBDIRECTORY) {
        ...
        *current_sector += num_sectors(node->file_size);
        ...
        traverse(node->subdirectory, TraversalMethod::PREFIX, [this](Node* node, void* context, int depth) {
            assign_offsets(node, static_cast<AssignOffsetsContext*>(context), depth);
        }, &ao_context, 0);
        
        traverse(node->subdirectory, TraversalMethod::PREFIX, [this](Node* node, void* context, int depth) {
            calculate_directory_offsets(node, static_cast<uint64_t*>(context), depth);
        }, current_sector, 0);
    }
}
...
void AvlTree::calculate_all() {
    ...
    traverse(&root_, TraversalMethod::PREFIX, [this](Node* node, void* context, int depth) {
        calculate_directory_requirements(node, context, depth);
    }, nullptr, 0);
...
uint64_t AvlTree::calculate_iso_size(Node* root_node) {
    ...
    traverse(root_node, TraversalMethod::PREFIX, [this](Node* node, void* context, int depth) {
        collect_nodes(node, static_cast<std::vector<Node*>*>(context), depth);
    }, &avl_nodes, 0);
