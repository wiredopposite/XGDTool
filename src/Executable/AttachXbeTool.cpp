...
void AttachXbeTool::generate_attach_xbe(const std::filesystem::path& out_xbe_path)
{
    ...
    std::memcpy(xbe_cert.title_name, title_helper_.title_name().data(), std::min(sizeof(xbe_cert.title_name), static_cast<size_t>(80)));

    out_file.seekp(exe_tool.cert_offset(), std::ios::beg);
    out_file.write(reinterpret_cast<const char*>(&title_helper_.xbe_cert()), sizeof(Xbe::Cert));
    if (out_file.fail()) 
    {
        throw XGDException(ErrCode::FILE_WRITE, HERE(), "Failed to write to file: " + out_xbe_path.string());
    }
